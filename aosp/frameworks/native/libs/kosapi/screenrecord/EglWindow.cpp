...
EglWindow::~EglWindow() {
    ALOGV("EglWindow()::~EglWindow---");
    eglRelease();
    ALOGV("---EglWindow()::~EglWindow X");
}
...
status_t EglWindow::makeCurrent() const {
    ALOGV("EglWindow::makeCurrent mEglDisplay: %p, mEglSurface: %p, mEglContext: %p", mEglDisplay, mEglSurface, mEglContext);
