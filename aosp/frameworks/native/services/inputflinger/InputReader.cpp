...
void InputDevice::process(const RawEvent* rawEvents, size_t count) {
    ...
    for (const RawEvent* rawEvent = rawEvents; count--; rawEvent++) {
        ...
        if (mDropUntilNextSync) {
            ...
            /** 飞鼠和蓝牙键盘(同时具有AINPUT_SOURCE_MOUSE和AINPUT_SOURCE_KEYBOARD)会导致点击enter键的时候触发onTouch事件，
            导致点击enter键触发的位置为光标所在位置而不是focus的位置，所以当ENTER键时对AINPUT_SOURCE_MOUSE进行过滤
            **/
            bool hasOther = false;
            if(rawEvent->code == KEYCODE_ENTER){
                for(size_t i = 0; i < numMappers; i++){
                    InputMapper* mapper = mMappers[i];
                    if(mapper->getSources() != AINPUT_SOURCE_MOUSE){
                        hasOther = true;
                        break;
                    }
                }
            ...
            for (size_t i = 0; i < numMappers; i++) {
                InputMapper* mapper = mMappers[i];
                //ALOGI("sjf mapper[%d]->getSources() %d.",i,mapper->getSources());
                //ALOGI("sjf rawEvent->code：%d，KEYCODE_ENTER：%d.",rawEvent->code,KEYCODE_ENTER);
                //
                //ALOGI("sjf AINPUT_SOURCE_MOUSE:%d,AINPUT_SOURCE_KEYBOARD:%d.",AINPUT_SOURCE_MOUSE,AINPUT_SOURCE_KEYBOARD);
                if(rawEvent->code == KEYCODE_ENTER){
                    if(mapper->getSources() == AINPUT_SOURCE_MOUSE && hasOther){
                  //      ALOGI("sjf mapper[%d] is Mouse",i);
                        continue;
                    }else if(mapper->getSources() == AINPUT_SOURCE_KEYBOARD){
                    //    ALOGI("sjf mapper[%d] is keyboard",i);
                    }
                }
...
void TouchInputMapper::dispatchWheelScroll(nsecs_t when, uint32_t policyFlags) {
    ...
    if (mCurrentCookedState.cookedPointerData.pointerCount != 1) {
        return;
    }
    const int32_t metaState = getContext()->getGlobalMetaState();
    nsecs_t downTime = mDownTime; // when
    // ALOGD("TouchInputMapper::dispatchWheelScroll--- rawVScroll: %i, rawHScroll: %i", mCurrentRawState.rawVScroll, mCurrentRawState.rawHScroll);
