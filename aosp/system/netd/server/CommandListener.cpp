...
CommandListener::CommandListener() :
    ...
    registerLockingCmd(new LauncherCommand(*this));
...
/*
void CommandListener::dispatchCommand(SocketClient *c, char *data)
{
    ALOGD("[netd]dispatchCommand #%u(%s)", number_ ++, data);
    FrameworkListener::dispatchCommand(c, data);
}
*/
void CommandListener::sendBroadcast(int code, const char *msg, bool addErrno)
{
    ALOGD("[netd]sendBroadcast(%i, %s, %s), sendBroadcast_: %s, mClients: %i", code, msg, addErrno? "true": "false", sendBroadcast_? "true": "flase", (int)mClients->size());
    if (sendBroadcast_) {
...
CommandListener::LauncherCommand::LauncherCommand(CommandListener& listener)
    : NetdCommand("launcher")
    , listener_(listener) {
}

int CommandListener::LauncherCommand::syntaxError(SocketClient* client, const char* message) {
    client->sendMsg(ResponseCode::CommandSyntaxError, message, false);
    return 0;
}

int CommandListener::LauncherCommand::operationError(SocketClient* client, const char* message,
                                                    int ret) {
    errno = -ret;
    client->sendMsg(ResponseCode::OperationFailed, message, true);
    return 0;
...
int CommandListener::LauncherCommand::success(SocketClient* client) {
    client->sendMsg(ResponseCode::CommandOkay, "success", false);
    return 0;
}

int CommandListener::LauncherCommand::runCommand(SocketClient* client, int argc, char** argv) {
    if (argc < 2) {
        return syntaxError(client, "Missing argument");
    }

    //    0     1         2
    // launcher broadcast enable
    // launcher broadcast <enable|disable>
    //
    // nexthop may be either an IPv4/IPv6 address or one of "unreachable" or "throw".
    if (!strcmp(argv[1], "broadcast")) {
        ...
        ALOGD("[netd]LauncherCommand::runCommand broadcast(%s)", enable? "enable": "disable");
        listener_.enableSendBroadcast(enable);

        return success(client);
    }

    //    0        1   2
    // launcher netid  <interface>
    if (!strcmp(argv[1], "netid")) {
        if (argc != 3) {
            return syntaxError(client, "Missing argument");
        }
        const char* interface = argv[2];
        unsigned existingNetId = gCtls->netCtrl.getNetworkForInterface(interface);
        // #define NETID_UNSET 0u  <aosp>/bionic/libc/dns/include/resolv_netid.h

        char *msg = NULL;
        asprintf(&msg, "%s %u", interface, existingNetId);

        client->sendMsg(ResponseCode::CommandOkay, msg, false);
        free(msg);
