...
import org.librose.SDLBle;
import org.librose.SDLWifi;
...
public class SDLActivity extends Activity implements View.OnSystemUiVisibilityChangeListener {
    ...
    // Ble
    protected static boolean mUseBle = false;
    protected static SDLBle mSDLBle = null;
    protected static boolean mUseWifi = false;
    protected static SDLWifi mSDLWifi = null;
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == SDLBle.REQUEST_ENABLE_BT && resultCode == Activity.RESULT_OK){
    ...
    // wifi
    //
    public static void wifiSetEnable(boolean enable) {
        mSDLWifi.enableWifi(enable);
    }

    public static int wifiGetFlags() {
        return mSDLWifi.wifiGetFlags();
    }

    public static List<SDLWifi.SDLWifiApInfo> wifiGetScanResults() {
        return mSDLWifi.wifiGetScanResults();
    }

    public static boolean wifiConnect(String ssid, String password) {
        return mSDLWifi.wifiConnect(ssid, password);
    }

    public static boolean wifiRemove(String ssid) {
        return mSDLWifi.wifiRemove(ssid);
    }

    //
    // use as center section
    //
    public static boolean bleInitCenter(Context context)
    {
        if (mSDLBle == null) {
            return false;
        }
        mSDLBle.bleInitCenter();
        return true;
    }

    public static void bleScanPeripherals(Context context, String uuid)
    {
        Log.e(TAG, "Trying to bleScanPeripherals, uuid: " + uuid + ", Device API-" + Build.VERSION.SDK_INT);
        mSDLBle.startScan();
    }
    ...
    public static void bleConnectPeripheral(String address)
    {
        Log.e(TAG, "Trying to bleConnectPeripherals: " + address);
        mSDLBle.connect(address);
    }

    public static void bleDisconnectPeripheral(String address)
    {
        Log.e(TAG, "Trying to bleDisconnectPeripheral: " + address);
        mSDLBle.disconnect(address);
    }

    public static void bleDiscoverServices(String address)
    {
        Log.e(TAG, "Trying to bleDiscoverServices: " + address);
        mSDLBle.discoverServices(address);
    }

    public static void bleClearCache(String address)
    {
        Log.e(TAG, "Trying to bleClearCache: " + address);
        mSDLBle.refreshDeviceCache(address);
    }

    public static void bleNotifyCharacteristic(String address, String service, String characteristic, boolean enable) {
        mSDLBle.setCharacteristicNotificationByUuid(address, enable, UUID.fromString(service), UUID.fromString(characteristic));
    }

    public static void bleWriteCharacteristic(String address, String service, String characteristic, byte[] value) {
        mSDLBle.writeCharacteristicByUuid(address, value, UUID.fromString(service), UUID.fromString(characteristic));
    }

    public static void bleReadCharacteristic(String address, String service, String characteristic) {
        mSDLBle.readCharacteristicByUuid(address, UUID.fromString(service), UUID.fromString(characteristic));
    }
 ...
 */
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if ((mUseWifi || mUseBle) && Build.VERSION.SDK_INT >= 23) {
        ...
        if (mUseBle) {
            mSDLBle = new SDLBle(mSingleton);
        }

        if (mUseWifi) {
            mSDLWifi = new SDLWifi(mSingleton);
            mSDLWifi.registerWifi();
        }
    }
